    ...
    app.bindEvents = function () {
        $('#save-record-modal button[data-action="save"]').click(function (e) {
            ...
            if ($('#save-record-modal input[name="action"]').val() == 'edit') {
                url = "user/edit/" + $('#save-record-modal input[name="id"]').val();
            }
            var successFunction = function (data) {
                ...
                if (data['status'] == 'error') {
                    ...
                    savePopup.find('input[name="first_name"]').val('');
                    savePopup.find('input[name="last_name"]').val('');
                    savePopup.find('input[name="email"]').val('');
                    savePopup.find('input[name="password"]').val('');
                    savePopup.find('input[name="confirm_password"]').val('');
                    savePopup.find('input[name="status"]').val('');
                    savePopup.find('input[name="id"]').val('');
                    savePopup.find('input[name="action"]').val('create');
                    savePopup.find('input[name="password"]').closest('.form-group').show();
                    savePopup.find('input[name="confirm_password"]').closest('.form-group').show();
            ...
            $.ajax({
                ...
                dataType: "json",
                data: {
                    first_name: $('#save-record-modal input[name="first_name"]').val(),
                    last_name: $('#save-record-modal input[name="last_name"]').val(),
                    email: $('#save-record-modal input[name="email"]').val(),
                    password: $('#save-record-modal input[name="password"]').val(),
                    confirm_password: $('#save-record-modal input[name="confirm_password"]').val(),
                    status: $('#save-record-modal input[name="status"]').val(),
                    id: $('#save-record-modal input[name="id"]').val()
                }
        ...
        $('#user-list').on('click', 'button[data-action="edit"]', function (e) {
            e.preventDefault();
            var successFunction = function (data) {
                var savePopup = $('#save-record-modal');
                savePopup.find('input[name="id"]').val(data['data']['id']);
                savePopup.find('input[name="first_name"]').val(data['data']['first_name']);
                savePopup.find('input[name="last_name"]').val(data['data']['last_name']);
                savePopup.find('input[name="email"]').val(data['data']['email']);
                savePopup.find('input[name="status"]').val(data['data']['status']);
                savePopup.find('input[name="action"]').val('edit');
                savePopup.find('input[name="password"]').closest('.form-group').hide();
                savePopup.find('input[name="confirm_password"]').closest('.form-group').hide();
                savePopup.modal("show");
            };
            $.ajax({
                type: "POST",
                cache: false,
                url: "user/edit/" + $(this).attr('data-id'),
                dataType: "json",
                data: {}
            }).done(successFunction).fail(function (xhr, result, status) {
                alert('GetPermissions ' + xhr.statusText + ' ' + xhr.responseText + ' ' + xhr.status);
        ...
        $('#delete-record-modal button[data-action="delete"]').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: "user/delete/" + $('#delete-record-modal').find('input[name="id"]').val(),
                dataType: "json",
