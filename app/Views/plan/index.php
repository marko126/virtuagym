...
<div id="content">
    <div class="container portfolio">
        ...
        <div class="row">
            <div class="col-md-12">
                <table class="table table-striped table-hover" id="plan-list">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Name</th>
        ...
        "columns": [
            ...
            //{"data": null, "defaultContent": "<td class='text-center'><div class='btn-group'><button class='btn btn-default' data-action='edit'><i class='fa fa-pencil'></i></button><button class='btn btn-default' data-action='delete'><i class='fa fa-trash'></i></button></div></td>"}
            {
                "render": function (data, type, full, meta) {
                    return "<td class='text-center'><div class='btn-group'><a href='/public/plan/show/" + full.id + "' class='btn btn-default'><i class='fa fa-eye'></i></a><button class='btn btn-default' data-id='" + full.id + "' data-action='edit'><i class='fa fa-pencil'></i></button><button class='btn btn-default' data-id='" + full.id + "' data-action='delete'><i class='fa fa-trash'></i></button></div></td>";
                }
            }
        ]
    });
    ...
    app.bindEvents = function () {
        ...
        $('#save-record-modal button[data-action="save"]').click(function (e) {
            ...
            if ($('#save-record-modal input[name="action"]').val() == 'edit') {
                url = "plan/edit/" + $('#save-record-modal input[name="id"]').val();
            }
        ...
        $('#plan-list').on('click', 'button[data-action="edit"]', function (e) {
            e.preventDefault();
            var successFunction = function (data) {
                var savePopup = $('#save-record-modal');
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: "plan/edit/" + $(this).attr('data-id'),
                dataType: "json",
                data: {}
            }).done(successFunction).fail(function (xhr, result, status) {
                alert('GetPermissions ' + xhr.statusText + ' ' + xhr.responseText + ' ' + xhr.status);
        ...
        $('#plan-list').on('click', 'button[data-action="delete"]', function (e) {
            e.preventDefault();
        ...
        $('#delete-record-modal button[data-action="delete"]').click(function (e) {
            ...
            $.ajax({
                type: "POST",
                cache: false,
                url: "plan/delete/" + $('#delete-record-modal').find('input[name="id"]').val(),
                dataType: "json",
