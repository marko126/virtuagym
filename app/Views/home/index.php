<!-- ======== @Region: #highlighted ======== -->
<div id="highlighted">
    <div class="container">
        <div class="header">
            <h2 class="page-title">
                <span>Page title</span>
                <small>Page short description</small>
            </h2>
        </div>
    </div>
</div>
<div id="content" class="demos">
    <div class="container">
        <!--@EXAMPLE: Content Region With No Sidebars-->
        <h3 class="title-divider">
            <span>Example 1: <span class="de-em">Content Region With No Sidebars</span></span>
        </h3>
        <div class="row">
            <div class="col-sm-12">
                <div class="demo-block">Content Region With No Sidebars</div>
            </div>
        </div>
        <!--@EXAMPLE: End of Content Region With No Sidebars-->
        <hr />
        <!--@EXAMPLE: Content Region With Left Sidebar-->
        <h3 class="title-divider">
            <span>Example 2: <span class="de-em">Content Region With Left Sidebar</span></span>
        </h3>
        <div class="row">
            <!--Sidebar-->
            <div class="col-sm-3 sidebar sidebar-left">
                <div class="demo-block">Sidebar</div>
            </div>
            <!--Content Area-->
            <div class="col-sm-9">
                <div class="demo-block">Content Region With Left Sidebar</div>
            </div>
        </div>
        <!--@EXAMPLE: Content Region With Left Sidebar-->
        <hr />
        <!--@EXAMPLE: Content Region With Right Sidebar-->
        <h3 class="title-divider">
            <span>Example 3: <span class="de-em">Content Region With Right Sidebar</span></span>
        </h3>
        <div class="row">
            <!--Content Area-->
            <div class="col-sm-9">
                <div class="demo-block">Content Region With Right Sidebar</div>
            </div>
            <!--Sidebar-->
            <div class="col-sm-3 sidebar sidebar-right">
                <div class="demo-block">Sidebar</div>
            </div>
        </div>
        <!--@EXAMPLE: Content Region With Right Sidebar-->
        <hr />
        <!--@EXAMPLE: Content Region With Both Sidebars-->
        <h3 class="title-divider">
            <span>Example 4: <span class="de-em">Content Region With Both Sidebars (left -> content -> Right)</span></span>
        </h3>
        <div class="row">
            <!--Sidebar Left-->
            <div class="col-sm-3 sidebar sidebar-left">
                <div class="demo-block">Sidebar</div>
            </div>
            <!--Content Area-->
            <div class="col-sm-6">
                <div class="demo-block">Content Region With Both Sidebars</div>
            </div>
            <!--Sidebar Right-->
            <div class="col-sm-3 sidebar sidebar-right">
                <div class="demo-block">Sidebar</div>
            </div>
        </div>
        <!--@EXAMPLE: End of Content Region With Both Sidebars-->
        <hr />
        <!--@EXAMPLE: Content Region With Both Sidebars Stacked-->
        <h3 class="title-divider">
            <span>Example 5: <span class="de-em">Content Region With Both Sidebars Stacked (content -> sidebar 1 -> sidebar 2)</span></span>
        </h3>
        <div class="row">
            <!--Content Area-->
            <div class="col-sm-6">
                <div class="demo-block">Content Region With Both Sidebars Stacked</div>
            </div>
            <!--Sidebar Left-->
            <div class="col-sm-3 sidebar sidebar-right">
                <div class="demo-block">Sidebar</div>
            </div>
            <!--Sidebar Right-->
            <div class="col-sm-3 sidebar sidebar-right">
                <div class="demo-block">Sidebar</div>
            </div>
        </div>
        <!--@EXAMPLE: End of Content Region With Both Sidebars Stacked-->
    </div>
</div>